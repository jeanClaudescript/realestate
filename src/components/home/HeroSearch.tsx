...
import { Search, MapPin, SlidersHorizontal, Building2, Trees, Home, Car, Sparkles } from 'lucide-react'
import { Button } from '@/components/ui/Button'
import { useNavigate } from 'react-router-dom'
import { kigaliNeighborhoods, districts } from '@/lib/rwanda'
import { useLocale } from '@/context/LocaleContext'

type Tab = 'houses' | 'cars' | 'buy' | 'rent' | 'land' | 'commercial'

const tabs: { id: Tab; labelKey: string; icon: typeof Home; href?: string }[] = [
  { id: 'houses', labelKey: 'hero.tab.houses', icon: Sparkles, href: '/houses' },
  { id: 'cars', labelKey: 'hero.tab.cars', icon: Car, href: '/cars' },
  ...
  { id: 'commercial', labelKey: 'hero.tab.commercial', icon: Building2 },
]
...
export function HeroSearch() {
  ...
  return (
    ...
    >
      <div className="glass-dark rounded-2xl sm:rounded-3xl p-2 shadow-luxury">
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-1 p-1 mb-2">
          {tabs.map(({ id, labelKey, icon: Icon }) => (
            <button
              key={id}
              type="button"
              onClick={() => setActiveTab(id)}
              className={`flex items-center justify-center gap-1.5 px-2 py-2.5 rounded-xl text-xs sm:text-sm font-medium min-h-[44px] transition-all ${
                activeTab === id
                  ? id === 'houses' || id === 'cars'
                    ? 'bg-brand-gold text-brand-charcoal ring-1 ring-brand-gold'
        ...
        {activeTab === 'cars' && (
          <p className="text-center text-white/60 text-sm px-4 py-2">{t('cars.heroHint')}</p>
        )}
        ...
        <div className="flex flex-col gap-2 p-2">
          ...
          >
            {activeTab === 'cars' ? t('cars.explore') : activeTab === 'houses' ? t('houses.explore') : t('hero.search')}
          </Button>
        </div>
      </div>
    </motion.div>
